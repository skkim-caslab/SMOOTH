...
class Matmul(Operator):
    ...
    def __call__(self, input1: Tensor, input2: Tensor, config_file) -> Tensor:
        # [bs, M, K] * [K, N] = [bs, M, N]
        # self.data_type.name # w4a8
        # input1.data_type.name # int8
        #print(input1.data_type.name) # int8 -> int4
        assert self.data_type == input1.data_type
        assert self.data_type == input2.data_type
        self.input1_shape = input1.shape
        self.input2_shape = input2.shape
...
#        print(l2_tiles.shape)
        #print("SKKIM simulating", M_l2_t, N_l2_t, K_l2_t, M_remain, N_remain, K_remain)
...
#            print("simulating", M,N,K, l1_tile_M,l1_tile_N,l1_tile_K)
...
#                    print("CHECK NAN3", current_batch_compute_cycle_count, temp_l1_tile_compute_cycle_count)
    ...
    #            file_path = "./Tiles/whole_tile_list.json"
                ...
                remained_file_path = os.path.join(process_dir, f"remained_tile_list_{process_id}.json")
        ...
        def simulate_l2_tile_compute_cycle_count(
            ...
            N_remain = N % l1_tile_N
            K_remain = K % l1_tile_K
#            print('simulate_l2_tile_compute_cycle_count')
#            print('l1_tile_M :', l1_tile_M)
#            print('M :', M)
#            print('l1_tile_N :', l1_tile_N)
#            print('N :', N)
#            print('l1_tile_K :', l1_tile_K)
#            print('K: ', K)
...
#                        print("SKKIM first load tile")
                        ...
                        if (m, n, k) == (0, 0, 0):
                            ...
                            if previous_batch_M_N_write_count > 0:
                                #print("write tile from sram") #write
                                if current_batch_M_K_read_count > 0:
                                    remained_amount, sram_status = sram.write_tile_from_sram(
                                        sram_status, previous_m_n_k, chiplet_module, ops_name, 0, loadable_amount
                                    )
                                    #print("write MK", sram_status) #write
                                if current_batch_K_N_read_count > 0:
                                    remained_amount, sram_status = sram.write_tile_from_sram(
                                        sram_status, previous_m_n_k, chiplet_module, ops_name, 1, loadable_amount
                                    )
                                    #print("write KN", sram_status) #write
                                if current_batch_M_N_read_count > 0:
                                    remained_amount, sram_status = sram.write_tile_from_sram(
                                        sram_status, previous_m_n_k, chiplet_module, ops_name, 2, loadable_amount
                                    )
                                    #print("write MN", sram_status) #write
                            else:
                                #print("free tile from sram")
                                if current_batch_M_K_read_count > 0:
                                    sram_status = sram.free_tile_from_sram(
                                        sram_status, previous_m_n_k, chiplet_module, ops_name, 0,
                                    )
                                if current_batch_K_N_read_count > 0:
                                    sram_status = sram.free_tile_from_sram(
                                        sram_status, previous_m_n_k, chiplet_module, ops_name, 1,
                                    )
                                if current_batch_M_N_read_count > 0:
                                    sram_status = sram.free_tile_from_sram(
                                        sram_status, previous_m_n_k, chiplet_module, ops_name, 2,
                                    )
...
#                print("CHECK NAN 0", hided_io_cycle_count, current_batch_compute_cycle_count) #502, 502

                skkim_io_byte = 0
#                print("??????????????????????", loadable_amount, current_batch_compute_cycle_count, chiplet_module.compute_module.l2_bandwidth_per_cycle ,chiplet_module.compute_module.core.systolic_array.input_word_size )
                if (m, n, k) == (0, 0, 0):
                    remained_amount, sram_status = sram.write_previous_ops_from_sram(
                        sram_status, ops_name, loadable_amount
                    )
...
#                    print("CHECK NAN", hided_io_cycle_count, current_batch_compute_cycle_count , chiplet_module.compute_module.l2_bandwidth_per_cycle , chiplet_module.compute_module.core.systolic_array.input_word_size)

#                print("[SKKIM] loadable_amount,", loadable_amount )
#                print("[SKKIM] before load,", sram_status)
#                print(f"sram status: {sram_status}")
#                print(f"loadable amount: {loadable_amount}")
                print()

                while(loadable_amount > 0):
                    #print("loadable amout > 0 ")
...
#                print("[SKKIM] after load,", sram_status)
...
#>>>>>>> origin/skkim_preload
                ...
                compute_time_tick += current_batch_compute_cycle_count
                io_time_tick += unhided_io_cycle_count
                print("accum_compute cycle :", compute_time_tick)
                print("accum_io cycle(X2) :", io_time_tick)
...
#                else:
                print(f"sram occupancy[%](Y2) : {sram.get_sramutil(sram_status)/ chiplet_module.compute_module.core.SRAM_size * 100:.3f}")
                print(f"sram status: {sram_status}")
...
#            print(">>>>>>",chiplet_module.compute_module.core.systolic_array_count)
    ...
    def simulate_systolic_array_cycle_count(
        ...
        #print("compute cycle calculate", M,N,K,array_height, array_width, mac_per_clock)
        util_rate = -1
