import itertools
from math import floor, log2
import json
import os

# Define possible values
#m_values = [1, 8, 16]
seq_len = 32*1024
model_dim = 4096 #model dimension
head_num = 32
m_values = [head_num]

#seq_len = 64*1024
#N_list = [16, 64, 2048]
N_list = [16, 64, 2048]

SRAM_size = 128*1024
double_buffer = 2

def get_N_list(max_num):
    final_list = []
    for idx,val in enumerate(N_list):
        if val <= max_num:
            final_list.append(val)
    if len(final_list) == 0:
        final_list = [1]
    return final_list
n_values = {
    "q_projection": get_N_list(model_dim),
    "k_projection": get_N_list(model_dim),
    "v_projection": get_N_list(model_dim),
    "q_mul_k": get_N_list(seq_len), #8192
    "softmax": [1024,2048], #8192
    "a_mul_v": get_N_list(model_dim//head_num), #128 = 2048/16
    "w0_projection": get_N_list(model_dim), #2048
    "w1_projection": get_N_list(4*model_dim), #8192
    "w2_projection": get_N_list(model_dim)
}
k_values = {
    "q_projection": model_dim,
    "k_projection": model_dim,
    "v_projection": model_dim,
    "q_mul_k": model_dim, #2048
    "softmax": None,
    "a_mul_v": head_num * seq_len, #131072
    "w0_projection": model_dim, #2048
    "w1_projection": model_dim, #2048
    "w2_projection": 4*model_dim
}


# Definition of operation flow
ops_sequence = [
    "q_projection", "k_projection", "v_projection", "q_mul_k", "softmax", 
    "a_mul_v", "w0_projection", "w1_projection", "w2_projection"
]

# Specify operation flow after Softmax
custom_next_ops = {
    "q_mul_k": "a_mul_v",
    "softmax": None # softmax has no next_ops_name
}
# Calculate the total number of files to be created
total_configs = 1
for op in ops_sequence:
    total_configs *= len(n_values[op])

print(f"A total of {total_configs} JSON files will be created.")

# Proceed after user confirmation
user_input = input("Shall we continue? (y/n): ").strip().lower()
if user_input != 'y':
    print("File creation has been cancelled.")
    exit()
#os.system('rm -rf ./double_tile_size/config*')
# Create a combination and save it as each file
config_index = 0
for m in m_values:
    n_combinations = itertools.product(*[n_values[op] for op in ops_sequence]) 
    # Only includes cases where k_values ​​is not None
    k_ops = [op for op in ops_sequence if k_values[op] is not None]
#    k_combinations = itertools.product(*[k_values[op] for op in k_ops])
    #print("N",len(list(n_combinations)))
    #print("K",len(list(k_combinations)))

    for n_comb in n_combinations:
        json_data = {}
        print("n comb:",n_comb)
        for idx, op in enumerate(ops_sequence):
            print(idx, op)
            if op != 'softmax': tmp_m = 1
            else: tmp_m = m

            l1_tile_M = tmp_m
            l1_tile_N = n_comb[idx]

            json_data[op] = {
                "l1_tile_M": l1_tile_M,
                "l1_tile_N": l1_tile_N,
            }

            if op in k_ops: # Add only if k_values ​​is not None
                l1_tile_k = (SRAM_size // double_buffer - l1_tile_M * l1_tile_N) // (l1_tile_M + l1_tile_N)
                l1_tile_k = min(l1_tile_k, k_values[op])
                l1_tile_k = floor(log2(l1_tile_k))
                l1_tile_k = 2**l1_tile_k
                #json_data[op]["l1_tile_K"] = k_comb[idx - len(ops_sequence) + len(k_ops)]
                json_data[op]["l1_tile_K"] = l1_tile_k
            
            if op in custom_next_ops:
                if custom_next_ops[op] is not None:
                    json_data[op]["next_ops_name"] = custom_next_ops[op]
            else:
                json_data[op]["next_ops_name"] = "end" if op == ops_sequence[-1] else ops_sequence[idx + 1]
            
        
        # Create individual JSON files
        filename = f"./sram_double_xla_sram128_seq32K_m6.7B/config_{config_index}.json"
        with open(filename, "w") as f:
            json.dump(json_data, f, indent=4)
        print(f"JSON file was created: {filename}")
        config_index += 1
