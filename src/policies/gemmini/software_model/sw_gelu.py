...
class GeLU(Operator):
    ...
    def compile_and_simulate(self, pcb_module: Device):
        ...
        #print("CHCHCHCH",M,total_flop_count)
        compute_latency = (
            total_flop_count
            / pcb_module.compute_module.core.vector_unit.total_vector_flops_per_cycle
            / pcb_module.compute_module.core_count
        ...
        io_cycle = io_latency * pcb_module.compute_module.clock_freq
        compute_cycle = compute_latency * pcb_module.compute_module.clock_freq

        print('total cycle(X) : ',  max(compute_cycle, io_cycle))
        print('compute cycle(X1) : ',  compute_cycle)
        print('VE cycle(X1) : ',  compute_cycle)
        print('io cycle(X2) : ',  io_cycle)
        print('current cycle(X3) : ',  max(compute_cycle, io_cycle))
        loadable_amount =  max(compute_cycle, io_cycle)-io_cycle
        if loadable_amount != 0:
            print("loadable cycle(X4) : ", loadable_amount)
        print('memory bw util[%](Y1) : ',  io_cycle/max(compute_cycle, io_cycle)*100)
        print('sram occupancy[%](Y2) : ',  (M * 2 * data_type.word_size) / pcb_module.compute_module.core.SRAM_size * 100)
        print('va util[%](Y3) : ',  100)
        return max(compute_cycle, io_cycle)
