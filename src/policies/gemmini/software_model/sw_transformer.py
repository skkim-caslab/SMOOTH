...
#        print("Check:", mha_output.shape)  # [16,1,2048]
    ...
    def compile_and_simulate(self, system: System):
        ...
        mha_layernorm_latency = (
            self.layer_norm0.compile_and_simulate(device)
        ...
        if self.use_flash_attention:
            ...
            softmax_latency = (
                self.A_softmax.compile_and_simulate(device)
...
#            print("SKKIM latency", q_mul_k_latency,softmax_latency, a_mul_v_latency)
        ...
        ffn_layernorm_latency = (
            self.layer_norm1.compile_and_simulate(device)
            + device.compute_module.overhead.layernorm
        )
        print()
        ...
        gelu_latency = (
            self.H_gelu.compile_and_simulate(device)
